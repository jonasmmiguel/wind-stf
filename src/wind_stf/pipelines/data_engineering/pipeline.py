    ...
    return Pipeline(
        [
            node(
                ...
                outputs="measurements_hourly",
            ),
            node(
                func=aggregate_temporally,
                name="Downsample",
                inputs="measurements_hourly",
                outputs="measurements_daily",
            ),
            node(
                func=filter_districts,
                name="Filter Districts",
                inputs="measurements_daily",
                outputs="measurements_daily_filtered",
            ),
            # node(
            ...
            node(
                ...
                inputs=["measurements_daily_filtered", "power_installed"],
                outputs="capacity_factors_daily",
            ),
            node(
                func=handle_anomalous_values,
                name="Prepare",
                inputs=["capacity_factors_daily"],
                outputs="capacity_factors_daily_noanomaly",
            ),
        ]
    )
